/* ============================================================
   nash-agents.js  —  v3.0
   ① Content Agent  — وارد کردن مطالب به بلاگ با Gemini AI رایگان
   ② Support Agent  — چت پشتیبانی هوشمند

   Content Agent: Google Gemini 1.5 Flash (رایگان)
   Support Agent: Gemini + decision tree fallback
   ✅ بدون سرور  |  ✅ GitHub Pages  |  ✅ Supabase
   ============================================================ */
...
(function () {
  ...
  /* ── Gemini Key — از DB یا sessionStorage ─────────────── */
  const GeminiKey = {
    get()    { return sessionStorage.getItem('nash_gemini_key') || ''; },
    set(k)   { sessionStorage.setItem('nash_gemini_key', k.trim()); },
    clear()  { sessionStorage.removeItem('nash_gemini_key'); },
    exists() { return !!this.get(); },
    async load() {
      if (this.exists()) return this.get();
      try {
        const res = await fetch(
          `${SB_URL}/rest/v1/settings?key=eq.gemini_api_key&select=value&limit=1`,
          { headers: {
              apikey: SB_ANON,
        ...
        if (key) { this.set(key); return key; }
      } catch {}
      return '';
    }
  };

  // برای backward compat با کدهای قدیمی‌تر
  const APIKey = GeminiKey;

  /* ── Gemini caller — با fallback روی چند مدل ──────────── */
  ...
  async function callAI({ system, userMsg, maxTokens = 800 }) {
    let key = GeminiKey.get();
    if (!key) key = await GeminiKey.load();
    ...
    for (const model of GEMINI_MODELS) {
      try {
        ...
        const res = await fetch(url, {
          method: 'POST',
          headers: { 'Content-Type': 'text/plain;charset=UTF-8' }, // از CORS preflight جلوگیری می‌کند
          body,
        });
        if (!res.ok) {
        ...
        if (data.promptFeedback?.blockReason) {
          throw new Error('محتوا توسط Gemini مسدود شد: ' + data.promptFeedback.blockReason);
        }
  ...
  const Content = (() => {
    ...
    async function process() {
      ...
      // key از DB یا sessionStorage
      const key = await GeminiKey.load();
      if (!key) {
        log('کلید Gemini API تنظیم نشده. از پنل ادمین → رسانه ذخیره کنید.','err');
        return;
      }
      ...
      log(`شروع پردازش ${toProcess.length} مقاله با Gemini AI...`);
      let done=0;
    ...
    function init() {
      document.body.insertAdjacentHTML('beforeend', `
        ...
        <div id="na-c-panel" dir="rtl">
          ...
          <div class="na-c-body">
            <div id="na-c-key-warning" style="display:none;background:rgba(248,113,113,0.1);border:1px solid rgba(248,113,113,0.3);border-radius:8px;padding:10px;font-size:11px;color:#f87171;margin-bottom:10px">
              ⚠️ کلید Gemini API تنظیم نشده. از پنل ادمین → رسانه، کلید را ذخیره کنید.
            </div>
            ...
            <div id="na-c-articles" style="display:none;margin-top:12px">
              ...
              <button class="na-btn na-btn-green" id="na-c-process">✨ پردازش با Claude AI و ذخیره در بلاگ</button>
            </div>
      ...
      syncBtn();
      // چک وجود Claude key
      APIKey.load().then(key => {
        const warn = document.getElementById('na-c-key-warning');
